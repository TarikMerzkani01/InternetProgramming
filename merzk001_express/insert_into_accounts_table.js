...
dbCon.connect(function (err) {
    ...
    const saltRounds = 10;
    const myPlaintextPassword = 'shadic'; // replace with plaintext password chosen by you OR retain the same value
    const passwordHash = bcrypt.hashSync(myPlaintextPassword, saltRounds);

    const rowToBeInserted = {
        acc_name: 'tarik',            // replace with acc_name chosen by you OR retain the same value
        acc_login: 'tarik',           // replace with acc_login chosen by you OR retain the same value
